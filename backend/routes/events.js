    ...
    try {
        ...
        if (event.teamSize === 1) {
            ...
            for (let i = 0; i < team.length; i++) {
                ...
                if (!user_events) {
                    return res.status(400).send({ 'message': 'The following roll number(s) are invalid or do not exist!' })
                }

                if (user_events.events.includes(event._id)) {
                    return res.status(400).send({ 'message': 'Your team member(s) have already registered for this event!' })
                }
            }
